...
export class CategoryService {
  ...
  async save(
    ...
    if (categoryDto.parentCategoryCode) {
      const parentCategory = await this.categoryRepository.findOne({
        code: categoryDto.parentCategoryCode,
      });

      if (!parentCategory) {
        throw new Error(
          `Invalid parent category ID = ${categoryDto.parentCategoryCode} provided.`,
        );
      }
  ...
  async findOne(where: Partial<Category>): Promise<CategoryDTO | undefined> {
