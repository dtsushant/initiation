import {
  ...
  RuleFieldMetadata,
  RuleInputFieldMetadata,
  RuleOutputFieldMetadata,
} from '../rule-field.decorator';

export function extractRuleFieldMetadata(
  instance: any,
): Record<string, RuleFieldMetadata> {
  const allFields = getRuleFieldMetaWithOptions(instance);

  return Object.entries(allFields).reduce(
    (acc, [key, meta]) => {
      const typeFn = meta.fnType;

      if (
        meta.type !== 'array' &&
        (meta.type === 'object' || !isPrimitive(meta.type)) &&
        typeFn
      ) {
...
function isOutputField(
  meta: RuleFieldMetadata,
): meta is RuleOutputFieldMetadata {
